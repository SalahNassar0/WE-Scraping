        ...
        try:
            ...
            try:
                ...
                for i in range(num_addon_cards):
                    ...
                    try:
                        price_locator_xpath = './/span[contains(normalize-space(.), "Price:")]'; price_elements = card.locator(f"xpath={price_locator_xpath}"); count = await price_elements.count()
                        if count > 0:
                            price_loc_instance = price_elements.first; actual_price_text = (await price_loc_instance.text_content(timeout=3000) or "").strip()
                            if "Price:" in actual_price_text:
                                parsed_price_val = actual_price_text.split("Price:", 1)[-1].strip()
                                if parsed_price_val: price_text_raw = parsed_price_val
                                else: price_text_raw = "N/A"
                            else: price_text_raw = "N/A"
                        else: price_text_raw = "N/A"
                    except Exception as e: price_text_raw = "N/A"
                    if name_text_cleaned != "N/A" and price_text_raw != "N/A" and "EGP" in price_text_raw:
                        ...
                        if price_numeric_part_for_name_cleaning and price_numeric_part_for_name_cleaning in name_text_cleaned:
                            patterns_to_remove = [r'-\s*' + re.escape(price_numeric_part_for_name_cleaning) + r'\s*EGP\s*/\s*month',r'-\s*' + re.escape(price_numeric_part_for_name_cleaning) + r'\s*EGP']
                            temp_name = name_text_cleaned
                            for pat in patterns_to_remove: temp_name = re.sub(pat, '', temp_name, flags=re.IGNORECASE).strip()
                ...
                if addon_names_list: addon_names_str = "; ".join(addon_names_list); addon_prices_str = "; ".join(addon_prices_list_scraped)
            if addon_names_str != "N/A": logging.info(f"Add-ons scraped for {account['phone']}: Names='{addon_names_str}', Prices='{addon_prices_str}'")
            cost_loc = page.locator('//span[contains(text(),"Renewal Cost")]/following-sibling::span//div[1]').first
            for attempt in range(1,4):
                try:
                    await cost_loc.wait_for(timeout=5000); val = (await cost_loc.text_content() or "").strip()
                    if val and val != "0": renewal_cost = val; break
                except Exception: 
                    if attempt <3: await page.wait_for_timeout(1000)
            if renewal_cost == "0": logging.warning(f"Could not reliably scrape Renewal Cost for {account['phone']}. Defaulting to 0."); renewal_cost = "0"
            date_loc = page.locator('//span[contains(text(),"Renewal Date:")]').first
            try:
                await date_loc.wait_for(timeout=5000); full_text_content = await date_loc.text_content() or ""
                match = re.search(r"Renewal Date:\s*([\d-]+)", full_text_content)
                if match: renewal_date = match.group(1).strip()
                else:
                    parts = full_text_content.split("Renewal Date:", 1)
                    if len(parts) > 1: renewal_date = parts[1].split(",")[0].strip()
                    else: renewal_date = ""
            except Exception: renewal_date = ""
        except Exception as e: logging.warning(f"Could not click 'More Details' or process subsequent info for {account['phone']}: {e}")
...
async def main():
    ...
    if 'Used' in df.columns: df["Used"] = pd.to_numeric(df["Used"], errors='coerce').fillna(0.0)
    else: df["Used"] = 0.0; logging.warning("'Used' column not found, defaulting to 0.")
    ...
    if SLACK_BOT_TOKEN and SLACK_CHANNEL_ID: 
        ...
        for index, row in df.iterrows():
            if pd.notna(row['Remaining']) and row['Remaining'] < LOW_REMAINING_RED_GB:
                message = (f":warning: *Low GB Alert!* Account: *{row['Store']}* ({row['Number']})\n"
        ...
        for index, row in df.iterrows():
            renewal_date_dt = row['Renewal Date DT']; balance_numeric = row['Balance Numeric']; total_cost_numeric = row['Total Cost Numeric']
            if pd.notna(renewal_date_dt):
                ...
                if days_to_renewal <= 5 and balance_numeric < total_cost_numeric:
                    ...
                    message = (f":alarm_clock: *Upcoming Renewal & Low Balance!* Account: *{row['Store']}* ({row['Number']})\n"
        ...
        if summary_window_start <= now <= summary_window_end:
            ...
            if low_gb_alert_count > 0: summary_report_parts.append(f":warning: *{low_gb_alert_count} account(s)* currently have Low GB (< {LOW_REMAINING_RED_GB} GB).")
            else: summary_report_parts.append(f":white_check_mark: All accounts currently OK for GB usage.")
            if renewal_low_balance_alert_count > 0: summary_report_parts.append(f":alarm_clock: *{renewal_low_balance_alert_count} account(s)* currently require attention for renewal and low balance.")
            else: summary_report_parts.append(f":white_check_mark: No immediate renewal/balance concerns at this time.")
            if not individual_alerts_to_send and low_gb_alert_count == 0 and renewal_low_balance_alert_count == 0 : summary_report_parts.append(f"\nOverall status: :thumbsup: Looking good! No specific issues detected.")
            else: summary_report_parts.append(f"\n_This is the consolidated daily summary. Issues detected are summarized above._")
            if TO_ADDRS and EMAIL_SENDER and EMAIL_PASSWORD: summary_report_parts.append(f"📧 _The detailed Excel report, containing full data for all accounts, has also been sent via email._")
            else: summary_report_parts.append(f"📧 _Email reporting is not configured._")
            daily_summary_message = "\n".join(summary_report_parts)
            send_slack_message(daily_summary_message)
            logging.info("Daily summary Slack report processing complete.")
        else:
            logging.info(f"Current time {now.strftime('%H:%M')} is outside the 12 PM summary window. Checking for individual alerts or sending 'All Clear'.")
