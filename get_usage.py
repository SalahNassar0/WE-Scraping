        ...
        for index, row in df[successful_scrape_mask].iterrows():
            if pd.notna(row['Renewal Date DT']) and \
               ...
               row['Renewal Cost'] != "Error EGP": # Additional check that renewal cost was actually scraped
                ...
                if days_to_renewal <= 15 and row['Balance Numeric'] < row['Total Cost Numeric']:
                    message = (f":alarm_clock: *Renewal/Low Bal!* Acct: *{row['Store']}* ({row['Number']}) - Renews: *{row['Renewal Date']}* ({days_to_renewal}d) - Bal: *{row['Balance Numeric']:.2f}*, Cost: *{row['Total Cost Numeric']:.2f}*"); 
